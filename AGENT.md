...
│                                       install.sh 首次生成，后续永远不覆盖
├── providers/                          生图 API 库
│   └── google.toml                     出厂唯一 provider: Google Gemini Image 官方 API
│                                       用户可自由添加任意三方桥接
└── demo/                               每次成品归档
    └── YYYY-MM-DD-HHMM-<slug>/         含时分，同日多 deck 不撞名
...
0. **Provider 就绪门 (强制, 第 0 步, 比库快照还早)** — 跑 `~/.claude/skills/ppt-anything/tools/check-providers.py`:
   - exit 0 + `READY: <名字>` → 有至少一个 provider 可用。记下名字, 第 2 步要拿来给用户选。
   - exit 1 + `NEED-CONFIG: ...` → **不许往下走**。立刻把首跑向导 (§0) 摆给用户: 路径 1 (自己 vim google.toml) / 路径 2 (你来调 register-provider.py 帮我写 + 验通)。直到 check-providers.py 重跑 exit 0 才回到第 1 步。

   **不许**装作没看到 NEED-CONFIG 还跑去问用户主题/张数 — 那叫白问, 后面 generate.py 必崩。
...
6. **生成模式由用户拍, 默认顺序**: 在第一次调用 generate.py 之前必须问"顺序还是批量"——顺序 = 推荐, 一张一张等返回, 安全; 批量 = 并发, 快但有 provider 风控/限频/封号风险, 选了必须警告 + 二次确认 + 上限 3 并发, 撞到 429 立即回退顺序。每张 slide 都把原始角色 ref + 上一张 slide ref 一起传给 provider。
7. **打包**: `tools/build-html-ppt-external.py`（默认 WebP 外链，HTML 壳 ~3KB）。
8. **归档**: 把 outline + slides + html + meta 写到 `~/.ppt-anything/demo/<YYYY-MM-DD>-<HHMM>-<slug>/`。含时分防止同日多 deck 撞名。
...
| 默认顺序, 批量必须用户主动选 + 警告 + 二次确认 + 上限 3 并发, 撞到限频立即回退 | API 风控直接封号 / 用户预算被烧 |
| 多 provider toml 时不许默认任一个，必须问用户 | 用户付错钱 / 走错 key / 走错 quality |
...
| 生图脚本 | `~/.claude/skills/ppt-anything/tools/generate-image.py`，凭证走 `~/.ppt-anything/providers/*.toml` |
...
项目出厂只装 `google.toml`（官方 Gemini）作为默认。用户可以自由接入任意三方桥接，项目不预设、不推荐、不限制具体三方服务商。

`generate-image.py` 支持两种 API 协议：Gemini-shape（generateContent）和 OpenAI-compatible（chat/completions）。具体的 `--provider` 选项和参数见 `generate-image.py --help`。

用户安装后想改默认 provider，直接改 `~/.claude/skills/ppt-anything/tools/generate-image.py` 里的 `DEFAULT_PROVIDER`。项目更新时 `install.sh` 会覆盖这个文件，用户需要重新改或维护自己的 patch。
